   ...
   <div class="row">
      <div  class="col-md-12 table-responsive" id="scrole">
         ...
         <div class="row">
            ...
            <button id="holi" class="btn {{$bg->custom}} btn-sm float-right" onclick="openiframe('Nueva conferencia','{{ route('conferencias.create')}}')">Crear conferencia</button>
            @endif -->
            <!-- <select class="my-1 form-control form-lavender w-20 d-inline" id="prioridad">
      ...
      <table id="table" class="table align-items-center mb-0">
         ...
         <tbody id="tbody">
            ...
            <tr id="{{$conferencia->id}}">
               ...
               <td class="td-short" title="{{$conferencia->nombre}}"><span id="cop" class="material-icons text-sm scale mr-2 pointer" title="Copiar" onclick="copyToClipboard()">content_copy</span>
                  <span id="cname">{{$conferencia->nombre}}</span></td>
                  ...
                  <td>
                     ...
                     <span class="text-estado bg-dark pointer" onclick="openiframe('Cancelación / reagendación conferencia','{{ route('cancelaciones.show',$conferencia->id)}}')"><i class="material-icons text-white text-sm scale">event_busy</i><span class="badge badge-light rounded-circle float-right {{isset($rescan[$key]) ? 'visible' : 'invisible'}}">&nbsp;</span></span>
                     @endif
                  </td>
               </tr>
               ...
               <tr id="{{$conferencia->id}}">
                  ...
                  <td>
                     ...
                     <span class="text-estado bg-dark pointer" onclick="openiframe('Solicitud de cancelación / reagendación','{{ route('cancelaciones.create',$conferencia->id)}}')"><i class="material-icons text-white text-sm scale">event_busy</i><span class="badge badge-light rounded-circle float-right {{isset($rescan[$key]) ? 'visible' : 'invisible'}}">&nbsp;</span></span>
                     @elseif($conferencia->estado==2)
                     <a href="{{ route('pdf_conf.show',$conferencia) }}" target="_blank"><span class="text-estado bg-info"><i class="material-icons text-white text-sm line-center">download</i>&nbsp;&nbsp;&nbsp;&nbsp;Agendada&nbsp;&nbsp;</span></a>
                     <span class="text-estado bg-primary pointer" onclick="openiframe('Participantes conferencia','{{ route('personas.show',$conferencia->id)}}')">
                        <i class="material-icons text-white text-sm scale">group</i>
...
<script type="text/javascript">
   ...
   function copyToClipboard() {
    ...
    range.selectNode(document.getElementById("cname"));
